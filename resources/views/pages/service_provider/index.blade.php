...
<div class="home">
    ...
    <div class="home_content">
        <div class="home_title">Penyedia Jasa</div>
    </div>
</div>
            ...
            <div class="col fill_height">
                ...
                <div class="search_tabs_container">
                    <div class="search_tabs d-flex flex-lg-row flex-column align-items-lg-center align-items-start justify-content-lg-between justify-content-start">
                        ...
                        <div class="search_tab active d-flex flex-row align-items-center justify-content-lg-center justify-content-start"><img src="{{ asset('images/diving.png') }}" alt="">Penyedia Jasa</div>
                    </div>
                </div>
                        ...
                        <div class="search_item" style="margin-top: 20px;">
                            ...
                            <select name="service_provider_type_id" id="service_provider_type_id" class="dropdown_item_select search_input" required="required">
                                ...
                                @foreach($service_provider_types as $service_provider_type)
                                    @php
                                        $selected = '';
                                        if(!empty($service_provider_type_id)){
                                            if($service_provider_type_id == $service_provider_type->id){
                                                $selected = 'selected';
                                            }
                                        }
                                    @endphp
                                    <option value="{{ $service_provider_type->id }}" {{ $selected }}>{{ $service_provider_type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <button class="button search_button">search<span></span><span></span><span></span></button>
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="offers_image_container">
                                    ...
                                    <div class="offers_image_background" style="background-image:url({{ asset($service_provider->default_picture) }})"></div>
                                    <div class="offer_name"><a href="{{ url('service_provider/'.$service_provider->id) }}">{{ $service_provider->name }}</a></div>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="offers_content">
                                    <div class="offers_price">Rp {{ number_format($service_provider->price, 0, ',', '.') }}</div>
                                    ...
                                    <p class="offers_text">{{ $service_provider->desc }}</p>
                                    ...
                                    <div class="offers_link"><i class="fa fa-arrow-right"></i> <a href="{{ url('service_provider/'.$service_provider->id) }}">read more</a> &nbsp;&nbsp;&nbsp; <a href="#" onclick="createOrder('{{ $service_provider->id }}', 'service_provider')"> <i class="fa fa-shopping-cart"></i> order now</a></div>
                                </div>
                            </div>
                        </div>
                    </div>
